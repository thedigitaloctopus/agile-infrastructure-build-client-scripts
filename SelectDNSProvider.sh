...
then
    ...
    do
        ...
        if ( [ ! -f ${BUILD_HOME}/buildconfiguration/${CLOUDHOST}/${BUILD_IDENTIFIER}-rackspace-credentials/DNSEMAILADDRESS ] )
        then
            status "Please input your rackspace Email Address"
            read DNS_EMAIL_ADDRESS
            /bin/echo ${DNS_EMAIL_ADDRESS} > ${BUILD_HOME}/buildconfiguration/${CLOUDHOST}/${BUILD_IDENTIFIER}-rackspace-credentials/DNSEMAILADDRESS
            DNS_EMAIL_ADDRESS="`/bin/cat ${BUILD_HOME}/buildconfiguration/${CLOUDHOST}/${BUILD_IDENTIFIER}-rackspace-credentials/DNSEMAILADDRESS`"
        else
            DNS_EMAIL_ADDRESS="`/bin/cat ${BUILD_HOME}/buildconfiguration/${CLOUDHOST}/${BUILD_IDENTIFIER}-rackspace-credentials/DNSEMAILADDRESS`"
