...
then
    ...
    /usr/bin/exo -O json dns show ${domainurl} | /usr/bin/jq --arg tmp_domain "${subdomain}" '.[] | select (.name == $tmp_domain) | .id'
    #Alternative
...
then
    ...
    #Official
    /usr/bin/vultr dns record list ${domainurl} | /bin/grep ${subdomain} | /usr/bin/awk '{print $1}'
fi
