...
then
    ...
    /usr/bin/exo vm template register ${autoscaler_name} --boot-mode $BOOTMODE --url ${url} --username debian --zone ${region} --checksum ${checksum} --description "Snapshot of an ADT autoscaler"
