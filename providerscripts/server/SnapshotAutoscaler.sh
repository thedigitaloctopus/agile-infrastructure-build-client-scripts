...
then
  ...
  #  snapshot_id="`/usr/bin/exo -O json  compute instance snapshot list  | /usr/bin/jq --arg tmp_instance_name "${autoscaler_name}" '(.[] | select (.instance | contains($tmp_instance_name)) | .id)' | /bin/sed 's/"//g'`"
  #  /usr/bin/exo compute instance snapshot export ${snapshot_id}
  #  . ${BUILD_HOME}/providerscripts/server/RegisterTemplateFromSnapshot.sh
  #  url_and_checksum="`/usr/bin/exo -O json compute instance snapshot export ${snapshot_id} | /usr/bin/jq '.url , .checksum' | /bin/sed 's/\"//g' |  paste - - | /usr/bin/awk '{print $1,"XXYYZZ",$2}' | /bin/sed 's/ //g'`"
